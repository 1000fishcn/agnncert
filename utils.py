# -*- coding: utf-8 -*-
"""
Created on Mon Jun 17 19:39:37 2024
...
def load_best_model(best_epoch, paper, dataset, model, eval_enabled):
    ...
    if best_epoch == -1:
        checkpoint = torch.load(
            f"./checkpoints/{paper}/{dataset}/best_model",
            weights_only=False,
        )
    else:
        checkpoint = torch.load(
            f"./checkpoints/{paper}/{dataset}/model_{best_epoch}",
            weights_only=False,
        )
...
def resolve_checkpoint_path(base_dir, robust_dir, paper, dataset, t_value):
    candidates = [
        os.path.join(base_dir, robust_dir, paper, dataset, str(t_value), "best_model"),
        os.path.join(base_dir, robust_dir, paper, dataset, str(t_value), "best_model.zip"),
    ]
